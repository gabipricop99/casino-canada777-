    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->Paytable[3] = [0,0,0,25,200,500];
            $this->Paytable[4] = [0,0,0,25,150,400];
            $this->Paytable[5] = [0,0,0,25,125,300];
            $this->Paytable[6] = [0,0,0,20,100,200];
            $this->Paytable[7] = [0,0,0,20,50,100];
            $this->Paytable[8] = [0,0,0,10,20,75];
            $this->Paytable[9] = [0,0,0,10,20,50];
...
$user->wager = $user->wager + ($sum * $this->game->getCategoryWagerPercent() / 100);
        ...
        public function GetPoints($currentPoint){
            $points = [5, 10, 15];
            $arr_points = [];
            if($currentPoint >= 15){
                array_push($arr_points, 3);
                array_push($arr_points, $currentPoint);
            }else{
                for($i = 0; $i < 3; $i++){
                    if($currentPoint < $points[$i]){
                        array_push($arr_points, $i);
                        array_push($arr_points, $points[$i]);
                        break;
                    }
                }
            }
