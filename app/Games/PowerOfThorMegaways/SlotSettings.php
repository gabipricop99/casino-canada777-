    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->Paytable[3] = [0,0,10,20,100,200,400];
            $this->Paytable[4] = [0,0,0,10,40,50,100];
            $this->Paytable[5] = [0,0,0,10,20,30,50];
            $this->Paytable[6] = [0,0,0,5,10,15,40];
            $this->Paytable[7] = [0,0,0,4,8,12,30];
            $this->Paytable[8] = [0,0,0,4,8,10,25];
            $this->Paytable[9] = [0,0,0,3,5,10,20];
            $this->Paytable[10] = [0,0,0,3,5,10,18];
            $this->Paytable[11] = [0,0,0,2,4,8,16];
...
$user->wager = $user->wager + ($sum * $this->game->getCategoryWagerPercent() / 100);
        ...
        public function GetLastReel($lastReel, $lastBinaryReel){            
            ...
            for($i = 5; $i >= 0; $i--){
                if($lastBinaryReel[5 - $i] > 0){
                    $lastReel[$i] = -1;
                }
                // $reels[floor((5 - $i) % 6)][0] = $lastReel[$i];
                $reels[$i][0] = $lastReel[$i];
            }
            for($i = 0; $i < 6; $i++){
                for($j = 7; $j >=2; $j--){
                    if($reels[$i][$j] == -1){
        ...
        public function GetReelStrips($winType, $slotEvent, $slotReelId, $scattercount, $isTumb = false, $lastTumbReelCount = [0, 0, 0, 0, 0, 0])
        {
            ...
            foreach( $_obf_reelStripCounts as $index => $value ) 
            {
                ...
                if($index == 7){
                    ...
                    for($k = 0; $k < 4; $k++){
                        if($reel['reel' . ($k + 2)][0] > 13){
                            $reel['reel' . ($k + 2)][0] = rand(7, 12);
                        }
                    }
                    if($reel['reel2'][0] == 13){
                        $reel['reel2'][0] = rand(7, 12);
                    }
                    $reel['reel6'][0] = 19;
                }else{
