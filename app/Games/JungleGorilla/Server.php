    ...
    class Server
    {
        public function get($request, $game, $userId) // changed by game developer
        {
            ...
            { 
                ...
                $slotSettings->SetGameData($slotSettings->slotId . 'ReelMuls', []);
                $slotSettings->SetGameData($slotSettings->slotId . 'BonusWin', 0);
                $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                $slotSettings->SetGameData($slotSettings->slotId . 'BonusMpl', 0);
                $slotSettings->SetGameData($slotSettings->slotId . 'Lines', 20);
                $slotSettings->setGameData($slotSettings->slotId . 'LastReel', [3,9,10,8,8,9,8,5,11,11,11,11,8,7,7]);
                if( $lastEvent != 'NULL' ) 
                {
                    $reelMuls = [];
                    foreach( $lastEvent->serverResponse->ReelMuls as $key=>$mul) 
                    {
                        $reelMuls[$key] = $mul;
                    }
                    $slotSettings->SetGameData($slotSettings->slotId . 'ReelMuls', $reelMuls);
                ...
                $reelMuls = $slotSettings->GetGameData($slotSettings->slotId . 'ReelMuls');
                $currentMuls = [1,1,1,1,1];
                $_obf_StrResponse = '';
                $str_muls = [];
                for($i = 0; $i < count($slotSettings->Bet); $i++){
                    $_index = $slotSettings->Bet[$i] * 1000;
                    if(isset($reelMuls[$_index])){                        
                        if($bet == $slotSettings->Bet[$i]){
                            $currentMuls = $reelMuls[$_index];
                        }else{
                            $subMuls = $reelMuls[$_index];
                            array_push($str_muls, '{"b":"'.($slotSettings->Bet[$i] * 20).'","v":{"0":"5~'.$subMuls[0].'~'.$subMuls[0].'","1":"5~'.$subMuls[1].'~'.$subMuls[1].'","2":"5~'.$subMuls[2].'~'.$subMuls[2].'","3":"5~'.$subMuls[3].'~'.$subMuls[3].'","4":"5~'.$subMuls[4].'~'.$subMuls[4].'"}}');
                        }
                    }
                }
                if(count($str_muls) > 0){
                    $_obf_StrResponse = '&accb=[' . implode(',', $str_muls) . ']';
                }
                $lastReelStr = implode(',', $slotSettings->GetGameData($slotSettings->slotId . 'LastReel'));
                $Balance = $slotSettings->GetBalance();
                $arr_accv = [];
                for($i = 0; $i < 5; $i++){
                    array_push($arr_accv, $currentMuls[$i] . '~'.$currentMuls[$i].'~5');
                }
            ...
            else if( $slotEvent['slotEvent'] == 'doSpin' ) 
            {
                ...
                $reelMuls = $slotSettings->GetGameData($slotSettings->slotId . 'ReelMuls');
                $currentMuls = [1, 1, 1, 1, 1];
                if(isset($reelMuls[($betline * 1000)])){
                    $currentMuls = $reelMuls[($betline * 1000)];
                }
                $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                $Balance = $slotSettings->GetBalance();
                ...
                for( $i = 0; $i <= 2000; $i++ ) 
                {
                    ...
                    for( $r = 1; $r <= 5; $r++ ) 
                    {
                        ...
                        if(rand(0, 100) < 5){
                            ...
                            $reels['reel'.$r][$pos] = 12;
                            $tmpMuls[$r - 1]++;
                            array_push($scatterPoses, '12~' . ($pos * 5 + ($r - 1)));
                            if($tmpMuls[$r - 1] > 5){
                                $tmpMuls[$r - 1] = 5;
                            }
                        }
                    ...
                    for( $r = 1; $r <= 5; $r++ ) 
                    {
                        for( $k = 0; $k <= 2; $k++ ) 
                        {
                            if($reels['reel'.$r][$k] == $wild){
                                array_push($wildPoses, [$k * 5 + $r - 1, $tmpMuls[$r - 1]]);
                            }
                        }
                    }
                    $_lineWinNumber = 1;
                    ...
                    for( $k = 0; $k < $lines; $k++ ) 
                    {
                        ...
                        for($j = 1; $j < 5; $j++){
                            ...
                            if($firstEle == $wild){
                                $firstEle = $ele;
                                array_push($subWilds, $j - 1);
                                $mul = $mul * $tmpMuls[$j - 1];
                                $lineWinNum[$k] = $lineWinNum[$k] + 1;
                            }else if($ele == $firstEle || $ele == $wild){
                                if($ele == $wild){
                                    $mul = $mul * $tmpMuls[$j];
                                    array_push($subWilds, $j);
                                }
                                $lineWinNum[$k] = $lineWinNum[$k] + 1;
                                if($j == 4){
                                    ...
                                    if($lineWins[$k] > 0){
                                        if($mul > 1){
                                            for($kk = 0; $kk < count($subWilds); $kk++){
                                                $reelWilds[$subWilds[$kk]] = 1;
                                            }
                                        }
                                        $totalWin += $lineWins[$k];
                                        $_obf_winCount++;
                ...
                for($k = 0; $k < 5; $k++){
                    if($reelWilds[$k] == 1){
                        array_push($arr_accv, $tmpMuls[$k] . '~1~5');
                        $currentMuls[$k] = 1;
                    }else{
                        $currentMuls[$k] = $tmpMuls[$k];
                        array_push($arr_accv, $tmpMuls[$k] . '~'.$tmpMuls[$k].'~5');
                    }
                }
                
                $slotSettings->SetGameData($slotSettings->slotId . 'LastReel', $lastReel);
                $slotSettings->SetGameData($slotSettings->slotId . 'initReel', $initReel);
                ...
                if(count($wildPoses) > 0){
                    ...
                    for($k = 0; $k < count($wildPoses); $k++){
                        array_push($poses, $wildPoses[$k][0]);
                        array_push($values, $wildPoses[$k][1]);
                    }
                    $otherResponse = $otherResponse . '&mbv='. implode(',', $values) .'&mbp=' . implode(',', $poses);
                }
                if(count($scatterPoses) > 0){
                    $otherResponse = $otherResponse . '&rwd=' . implode(',', $scatterPoses);
                }
                ...
                $reelMuls[($betline * 1000)] = $currentMuls;
                $slotSettings->SetGameData($slotSettings->slotId . 'ReelMuls', $reelMuls);

                $_GameLog = '{"responseEvent":"spin","responseType":"' . $slotEvent['slotEvent'] . '","serverResponse":{"BonusMpl":' . 
                    $slotSettings->GetGameData($slotSettings->slotId . 'BonusMpl') . ',"lines":' . $lines . ',"bet":' . $betline . ',"Balance":' . $Balance . ',"afterBalance":' . $slotSettings->GetBalance() . ',"totalWin":' . $totalWin . ',"bonusWin":' .$slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') . ',"winLines":[],"Jackpots":""' . ',"ReelMuls":'.json_encode($reelMuls).',"LastReel":'.json_encode($lastReel).'}}';
