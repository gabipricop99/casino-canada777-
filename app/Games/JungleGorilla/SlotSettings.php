    ...
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->Paytable[2] = [0,0,0,30,50,250];
            $this->Paytable[3] = [0,0,0,25,40,90];
            $this->Paytable[4] = [0,0,0,20,35,70];
            $this->Paytable[5] = [0,0,0,15,30,60];
            $this->Paytable[6] = [0,0,0,12,24,50];
            $this->Paytable[7] = [0,0,0,10,20,40];
            $this->Paytable[8] = [0,0,0,10,18,35];
            $this->Paytable[9] = [0,0,0,8,16,30];
            $this->Paytable[10] = [0,0,0,6,12,25];
...
$user->wager = $user->wager + ($sum * $this->game->getCategoryWagerPercent() / 100);
        ...
        public function GetSpinSettings($garantType = 'doSpin', $bet, $lines)
        {
            ...
            // }
            if( $_obf_winline_count <= $_obf_grantwin_count ) 
            {
                $spinWin = 1;
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            foreach( $_obf_reelStripCounts as $index => $value ) 
            {
                ...
                if($isScatter == true){
                    ...
                    $reel['reel' . $index][-1] = $key[($value - 1 - $scatterPos) % $rc];
                    $reel['reel' . $index][0] = $key[($value - $scatterPos) % $rc];
                    $reel['reel' . $index][1] = $key[($value + 1 - $scatterPos) % $rc];
                    $reel['reel' . $index][2] = $key[($value + 2 - $scatterPos) % $rc];
                    $reel['reel' . $index][3] = $key[($value + 3 - $scatterPos) % $rc];
                }else{
                    $reel['reel' . $index][-1] = $key[($value - 1) % $rc];
                    $reel['reel' . $index][0] = $key[$value % $rc];
                    $reel['reel' . $index][1] = $key[($value + 1) % $rc];
