        ...
        public function __construct($sid, $playerId, $credits = null)
        {
           /* if( config('LicenseDK.APL_INCLUDE_KEY_CONFIG') != 'wi9qydosuimsnls5zoe5q298evkhim0ughx1w16qybs2fhlcpn' ) 
            ...
            $this->money_respin = [
                ...
                [25,50,75,100,125,150,200,250,375,625,1250,2500,12500]
            ];
            $this->money_jackpot = [
                [995, 3, 1, 1],
                [0, 2, 3, 5]
...
$user->wager = $user->wager + ($sum * $this->game->getCategoryWagerPercent() / 100);
        ...
        public function GetMoneyWin($isJackpot){
            if($isJackpot == true){
                $percent = rand(0, 90);
            }else{
                $percent = rand(0, 100);
        ...
        public function GetJackpotMoney($isJackpot){
            $percent = rand(0, 1000);
            $sum = 0;
            for($i = 1; $i < count($this->money_jackpot[0]); $i++){
                $sum = $sum + $this->money_jackpot[0][$i];
                if($sum >= $percent){
                    if($this->money_jackpot[1][$i] == 0 || $isJackpot == true){
                        return $this->GetMoneyWin($isJackpot);
                    }else{
        ...
        public function GetMoneyText($_moneyValue){
            $CurrentMoneyText = [];
            for($i = 0; $i < count($_moneyValue); $i++){
                if($_moneyValue[$i] == 0){
                    $CurrentMoneyText[$i] = 'r';
                }else if($_moneyValue[$i] <= 5){
                    $CurrentMoneyText[$i] = 'ma';
                }else if($_moneyValue[$i] == 1250){
                    $CurrentMoneyText[$i] = 'jp3';
                }else if($_moneyValue[$i] == 2500){
                    $CurrentMoneyText[$i] = 'jp2';
                }else if($_moneyValue[$i] == 12500){
                    $CurrentMoneyText[$i] = 'jp1';
                }else{
                    $CurrentMoneyText[$i] = 'v';
                }
            }
            return $CurrentMoneyText;
        }
        ...
        public function GetRandomScatterPos($rp, $reelNum)
        {
            $_obf_scatterposes = [];
            ...
            {
                $_obf_scatterposes[0] = rand(0, count($rp) - 4);
            }
        ...
        public function GetReelStrips($winType, $slotEvent)
        {
            ...
            if($slotEvent=='freespin'){
                ...
                else
                {
                    ...
                    for( $i = 0; $i < count($_obf_reelStripNumber); $i++ ) 
                    {
                        ...
                        {
                            $isScatter = true;
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = $this->GetRandomScatterPos($this->{'reelStripBonus' . $_obf_reelStripNumber[$i]}, $i);
                        }
                        else
                        {
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = rand(0, count($this->{'reelStripBonus' . $_obf_reelStripNumber[$i]}) - 4);
                        }
                    }
                }
            }else{
                ...
                else
                {
                    ...
                    for( $i = 0; $i < count($_obf_reelStripNumber); $i++ ) 
                    {
                        ...
                        {
                            $isScatter = true;
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = $this->GetRandomScatterPos($this->{'reelStrip' . $_obf_reelStripNumber[$i]}, $i);
                        }
                        else
                        {
                            $_obf_reelStripCounts[$_obf_reelStripNumber[$i]] = rand(0, count($this->{'reelStrip' . $_obf_reelStripNumber[$i]}) - 4);
                        }
                    }
                }
            }
            ...
            foreach( $_obf_reelStripCounts as $index => $value ) 
            {
                ...
                if($isScatter == false){
                    ...
                    $reel['reel' . $index][0] = $key[$value - $scatterPos];
                    $reel['reel' . $index][1] = $key[$value + (1 - $scatterPos)];
                    $reel['reel' . $index][2] = $key[$value + (2 - $scatterPos)];
                    $reel['reel' . $index][3] = $key[$value + (3 - $scatterPos)];
                    $reel['reel' . $index][4] = $key[$value + (4 - $scatterPos)];
                }
                if($index == 1 || $index == 2){
                    $reel['reel' . $index][3] = 13;
                }
                $reel['rp'][] = $value;
            }
            return $reel;
