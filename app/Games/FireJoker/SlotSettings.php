...
namespace VanguardLTE\Games\FireJoker
{
    class SlotSettings
    {
        ...
        public function __construct($sid, $playerId, $credits = null)
        {
            ...
            $this->scaleMode = 0;
            $this->numFloat = 0;
            $this->Paytable = [2, 4, 5, 6, 7, 15, 20, 25, 80];
...
$user->wager = $user->wager + ($sum * $this->game->getCategoryWagerPercent() / 100);
        ...
        public function GetBonusMul(){
            ...
            $percent = rand(0, 100);
            $sum = 0;
            for($i = 0; $i < count($bonusSpins[0]); $i++){
                $sum = $sum + $bonusSpins[0][$i];
                if($percent <= $sum){
                    return $bonusSpins[1][$i];
                }
            }
            return $bonusSpins[1][0];
        }
        public function GetBonusPos($mul){
            $muls = [10, 2, 4, 3, 5, 2, 4, 2, 3, 5, 3, 2];
            $poses = [];
            $count = 0;
            for($i = 0; $i < 12; $i++){
                if($muls[$i] == $mul){
